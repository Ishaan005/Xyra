...
import AgentBasedForm from "./AgentBasedForm"
...
export default function AgentModelDetail({ modelId, onBack }: AgentModelDetailProps) {
    ...
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">Loading agent model...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {editMode ? (
        <Card>
          <CardHeader>
            <CardTitle>Edit Agent Billing Model</CardTitle>
          </CardHeader>
          <CardContent>
            <AgentBasedForm model={model} setModel={setModel} />
        ...
        <Card>
          <CardHeader>
            <CardTitle>Agent Model Details</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              ...
              {model.agent_config && model.agent_config.length > 0 && (
                <div>
                  <strong>Agent Configuration:</strong>
                  <pre className="bg-gray-100 p-2 rounded mt-2 text-sm">
                    {JSON.stringify(model.agent_config[0], null, 2)}
                  </pre>
                </div>
              )}
            </div>
